...
1. Node.js や AWS を使って自前でサーバーとバックエンドを構築する
   現在は Supabase という BaaS を活用していますが、Node.js と AWS を使って自分でバックエンドとサーバーを一から設計・構築してみたいと考えています。
2. React Native を使ってネイティブアプリを開発する
   「EMOM Path」は現在 Web アプリとして提供していますが、今後は React Native を活用してスマートフォン向けのネイティブアプリ化にも挑戦したいです。
